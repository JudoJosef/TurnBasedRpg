...
public class DryadSkills : IChampionSkills
{
    ...
    {
        var deadCreatures = creatures.Where(creature => creature.Health <= 0).ToList();
        if (deadCreatures.Any())
        {
            var target = DungeonUtility.GetAlly(deadCreatures);
        ...
        {
            UiReferencer.WriteLineAndWait("No dead allies");
        }
    }
